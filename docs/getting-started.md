...
```sh
npx --yes guildctl@0.4.0 onboard \
  --host codex \
...
```sh
npx --yes guildctl@0.4.0 preset install server-observer \
  --application-id YOUR_APPLICATION_ID \
  --guild-id YOUR_GUILD_ID \
...
```sh
npx --yes guildctl@0.4.0 config validate ./guildcontrol.json
npx --yes guildctl@0.4.0 doctor --config ./guildcontrol.json --online
npx --yes guildctl@0.4.0 smoke --config ./guildcontrol.json
```
...
After setup reports `ready`, use the one-click MCPB path when the host supports it and the policy names an environment credential. Download `guildcontrol-0.4.0.mcpb` from the [immutable GitHub Release](https://github.com/j-256/guildcontrol/releases) or select the MCPB distribution from the MCP Registry, then:
...
```sh
npx --yes guildctl@0.4.0 host detect
```
...
```sh
npx --yes guildctl@0.4.0 host --npx --config ./guildcontrol.json --html ./guildcontrol-host-activation.html
```
...
```sh
npx --yes guildctl@0.4.0 host --npx --config ./guildcontrol.json --adapter vscode
```
...
```sh
npx --yes guildctl@0.4.0 host plan \
  --npx \
  --config ./guildcontrol.json \
  --adapter vscode \
...
```sh
npx --yes guildctl@0.4.0 host apply \
  --npx \
  --config ./guildcontrol.json \
  --adapter vscode \
  --host-file /absolute/path/to/mcp.json \
...
npx --yes guildctl@0.4.0 host --npx --config ./guildcontrol.json --adapter vscode --inspect-host-file /absolute/path/to/mcp.json
```
...
```sh
npx --yes guildctl@0.4.0 recipe enable message-channel ./guildcontrol.json \
...
```sh
npx --yes guildctl@0.4.0 config validate ./guildcontrol.json
npx --yes guildctl@0.4.0 doctor --config ./guildcontrol.json
npx --yes guildctl@0.4.0 doctor --config ./guildcontrol.json --online
npx --yes guildctl@0.4.0 smoke --config ./guildcontrol.json
npx --yes guildctl@0.4.0 host detect
npx --yes guildctl@0.4.0 host --npx --config ./guildcontrol.json --html ./guildcontrol-host-activation.html
npx --yes guildctl@0.4.0 host plan --npx --config ./guildcontrol.json --adapter ADAPTER_ID --host-file HOST_JSON_FILE
npx --yes guildctl@0.4.0 host apply --npx --config ./guildcontrol.json --adapter ADAPTER_ID --host-file HOST_JSON_FILE --plan-digest PLAN_DIGEST --confirm HOST_SERVER_NAME
npx --yes guildctl@0.4.0 host --npx --config ./guildcontrol.json --adapter ADAPTER_ID --inspect-host-file HOST_JSON_FILE
```

- If a bare `guildctl` command is not found, use the pinned `npx --yes guildctl@0.4.0` prefix or install the package globally before using the bare executable.
