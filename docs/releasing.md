...
4. In Settings, Pages, keep GitHub Actions as the publishing source by following [GitHub's custom-workflow instructions](https://docs.github.com/en/pages/getting-started-with-github-pages/using-custom-workflows-with-github-pages). Follow [GitHub's custom-domain instructions](https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site/managing-a-custom-domain-for-your-github-pages-site) to set `guildcontrol.lasers.app`, point its DNS `CNAME` directly to `j-256.github.io` without a repository path, and enforce HTTPS after GitHub provisions the certificate. A custom Actions workflow ignores repository `CNAME` files, so Pages settings own the domain binding. Restrict the `github-pages` environment to protected `main`, let the `Publish documentation portal` job deploy the exact browser-verified artifact, then run `node scripts/check-public-documentation.mjs` and require an exact result. After that verification succeeds, set the repository homepage to `https://guildcontrol.lasers.app`.
