...
The finite local public-only catalog compiles `community`, `creator`, `project`, and `support` into caller-retained requests with compact structure, symbolic category and per-parent child order, conservative settings, and an optional guild name. Compilation performs no discovery, policy inspection, network access, mutation, persistence, or authority grant. A scaffold may bind one logical-name candidate only on a complete live match; ambiguity, mismatch, visibility limits, or drift block. Starters omit roles, exact existing-resource metadata and permissions, private or read-only areas, Community, Welcome Screen, onboarding, AutoMod, and publication. After IDs are proven, add reviewed role order or permission overwrites, customize `channelOrders`, or author another layout. The `guild-starter` recipe preserves the live guild name, so `guildName` needs separate profile policy. Its shared scaffold and ordering authority can serve custom manifests: retain the compiled request and least-privilege bot permissions because an existing `Manage Roles` grant can permit role creation, while only an explicitly acknowledged channel chain can reparent.
...
- Discord task coordination is caller-driven, bounded, and non-persistent. It is not a queue or identity system; shared-bot reactions cannot authenticate or authorize separate sessions
