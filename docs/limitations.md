...
| Caller-supplied remote URL, arbitrary media fetch, or raw attachment forwarding | External inputs add credential, tracking, substitution, and content risks | For inbound guild content, select exact channel, message, and attachment IDs so the connector can bind a fresh Discord-supplied signed URL internally; for outbound files, use only a separately enabled bounded local-file workflow where one exists |
