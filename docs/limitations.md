...
The bundled guild-blueprint starter catalog is finite, local, and public-only. `community`, `creator`, `project`, and `support` compile compact public structure, conservative settings that preserve the live verification level, and an optional guild name into the strict caller-retained request without discovery, policy inspection, authority, network access, or mutation. Scaffold planning may bind only one unambiguous logical-name candidate whose complete live state matches; duplication, mismatch, visibility limits, or drift block. Starters omit role creation, existing-resource changes, private or read-only areas, visual order, Community, Welcome Screen, onboarding, AutoMod, and publication. Review the omissions and warnings. Once exact IDs are proven, add role-order or permission-overwrite convergence to the retained manifest and preview it again, or use the standalone exact-ID workflows; author a custom blueprint for other layouts. The narrower `guild-starter` recipe preserves the live guild name, so optional `guildName` requires separate profile policy. Its recipe cannot narrow the shared scaffold schema: retain the compiled request and least-privilege bot permissions because another manifest may request roles when the bot has `Manage Roles`.
...
- Guild blueprint exact role, hierarchy, channel, and permission-overwrite intent is same-guild and caller-authored. Capture omits `roleConfigurations`, `roleOrder`, `channelMetadata`, and `channelPermissionOverwrites`. Role order accepts exact or receipt-bound scaffold roles; overwrites require an allowlisted exact channel plus an exact member or exact or receipt-bound role. Channel ordering and parent synchronization remain separate. A matching receipt proves only fresh exact state; drift needs new reviewed intent and key
