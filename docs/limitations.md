...
`host --npx --config FILE --html PRIVATE_FILE` emits a credential-free pinned launch contract plus adapters for Claude Code, Codex, Cursor, VS Code, Gemini CLI, and common MCP JSON. Each binds its configuration, destination, secret strategy, requirements, limitations, and schema source to the activation digest. The private guide adds copy controls and a read-only verification request without contacting a network, starting or editing a host, or opening a browser. It may contain Discord IDs, local paths, or an encoded Cursor URI, so do not share or commit it.

| Generated adapter | Supported handoff | Important boundary |
| --- | --- | --- |
| `claude-code` | Project `.mcp.json` or managed scope | Exact `${NAME}` expansion; the projection does not choose or edit a scope |
| `codex` | User or project `config.toml` | Named environment forwarding and host gates; TOML remains a manual merge target |
