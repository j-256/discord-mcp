...
The bundled guild-blueprint starter catalog is likewise finite, local, and public-only. `community`, `creator`, `project`, and `support` compile compact categories, ordinary text channels, forums, conservative named settings that preserve the live verification level, and an optional guild name into the strict caller-retained planner request. Compilation does not discover or select live resources. Later scaffold planning may bind one unambiguous logical-name candidate only when its complete current state exactly matches the additive request; duplicates, mismatches, visibility limits, or drift block. A starter does not mutate an existing role or channel, create roles, make an information channel read-only, create a private staff area, set final visual order, enable Community, populate a Welcome Screen or onboarding, configure AutoMod, publish a message, inspect policy, grant authority, or contact Discord. Review its explicit omissions and warnings, use the separate exact-ID permission and ordering workflows after new IDs are proven where needed, and use custom blueprint authoring when the fixed layout does not fit. The narrower `guild-starter` recipe preserves the live guild name, so optional `guildName` input requires separate profile policy. It also does not convert the shared scaffold capability into a starter-only schema; retain the compiled request and least-privilege bot permissions because a separately authored manifest can still request roles when the bot already has `Manage Roles`.

Guild-blueprint preview is a complete view of normalized caller intent, dependencies, references, and possible phase stages, not a live whole-guild simulation. It deliberately does not contact Discord, decide which writes are required, resolve scaffold keys into future IDs, or predict permissions, hierarchy, capacity, receipts, and state after an earlier write. Live planning overlays every intent entry with freshly assessed or deferred status and exposes only one executable frontier. A complete single-snapshot operation list would be easier to apply in bulk, but it would blur which later decisions depend on IDs and state that do not yet exist.
...
- Guild blueprint exact-role and exact-channel convergence is same-guild and caller-authored. Capture does not infer those mutation phases, and the manifest does not converge role or channel ordering or permission overwrites. A completed matching nested receipt can prove only fresh exact current state; later drift requires new reviewed intent and a new key rather than replay
...
| Name-adopting or broad best-effort declarative reconciliation | A logical match can select the wrong duplicate, while partial retry after ambiguous writes can repeat effects | Use exact-ID blueprint convergence for supported role and channel fields, and the separate reviewed ordering or permission workflows for broader structural changes |
