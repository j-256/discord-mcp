...
An allowlisted channel grants local read scope to child threads, including forum posts, but does not grant deletion or pin-management scope to those thread IDs or forum-post creation scope to another channel. Pin changes require the target thread's own exact ID in the pin allowlist, and forum-post creation requires the parent forum's own exact ID in its separate allowlist. When a channel allowlist is configured, guild search is constrained to exact allowed channel IDs before contacting Discord.
