...
function applicationIntentReceipt(
  ...
  return {
    ...
    operationKeyHash: operationKeyHash("application-intent-operation-0001"),
    planDigest: PLAN_DIGEST,
    resourceId: ["completed", "uncertain"].includes(status) ? GUILD_ID : null,
    schemaVersion: 1,
    status,
    timestamp: status === "pending"
      ? "2026-08-24T00:00:00.000Z"
...
test("file operation store keeps application intent receipts content-free", async (context) => {
  ...
  const directory = join(root, "receipts")
  const store = new FileOperationStore(directory)
  const pending = applicationIntentReceipt()
  const completed = applicationIntentReceipt("completed")
  ...
  assert.match(text, /application-intent-enablement/)
  assert.doesNotMatch(text, /review reason|raw flags|application-intent-operation-0001/)

  await assert.rejects(
    () => store.finishApplication({
      ...completed,
      resourceId: "999999999999999999",
    }),
...
test("file operation store isolates every durable write operation-key domain", async (context) => {
  ...
  const guildCommunity = { ...receipt(), kind: "guild-community-change" as const }
  const guildDeparture = { ...receipt(), kind: "guild-departure" as const }
  ...
  const soundboardPlayback = { ...receipt(), kind: "soundboard-playback" as const }
  ...
  assert.equal((await store.reserve(guildCommunity)).created, true)
  assert.equal((await store.reserve(guildDeparture)).created, true)
  ...
  assert.equal((await store.reserve(soundboardPlayback)).created, true)
  ...
  assert.deepEqual(
    await store.get("guild-community-change", guildCommunity.operationKeyHash),
    guildCommunity,
  )
  assert.deepEqual(
    await store.get("guild-departure", guildDeparture.operationKeyHash),
    guildDeparture,
  )
  assert.deepEqual(
  ...
  assert.deepEqual(
    await store.get("member-moderation", memberModeration.operationKeyHash),
    memberModeration,
  )
  assert.deepEqual(
  ...
  assert.deepEqual(
    await store.get("soundboard-playback", soundboardPlayback.operationKeyHash),
    soundboardPlayback,
  )
  assert.deepEqual(
