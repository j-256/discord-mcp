...
The `server-observer` preset exposes guild metadata, roles, permission diagnostics, connector health, content-free activity, and tool discovery. It cannot enable writes, the Gateway, telemetry export, activity persistence, or Message Content access. Setup stores the credential reference and verified public IDs, never the token value. The credential-free `host` command turns that policy into an exact pinned stdio contract and optional mode-0600 interactive guide without contacting Discord, starting a process, or editing host configuration. Use its copyable semantic field mapping and read-only verification request with any compatible local host. `route_discord_goal` gives every prompt-capable client one safe entry point for later discovery, bounded reads, and at most a reviewed plan for a write, never a mutation.
...
| `discord-mcp host --npx --config FILE --html PRIVATE_FILE` | Exact credential-free policy-to-stdio mapping and private interactive activation guide without host mutation | None |
