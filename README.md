...
<img src="https://raw.githubusercontent.com/j-256/guildcontrol/v0.4.0/assets/guildcontrol-icon.png" alt="GuildControl MCP shield and reviewed connection icon" width="128">
...
```sh
npx --yes guildctl@0.4.0 catalog --check
npx --yes guildctl@0.4.0 catalog --html ./guildcontrol-contract.html
npx --yes guildctl@0.4.0 preset show server-observer
```
...
```sh
npx --yes guildctl@0.4.0 migrate list
npx --yes guildctl@0.4.0 migrate plan cappyeo@0.26.0 --html ./guildcontrol-migration.html
```
...
```sh
npx --yes guildctl@0.4.0 config workbench \
  ./guildcontrol.json \
  --html ./guildcontrol-workbench.html
npx --yes guildctl@0.4.0 config replace \
  ./guildcontrol.json \
...
```sh
npx --yes guildctl@0.4.0 config plan \
  ./guildcontrol.json \
  ./guildcontrol.candidate.json
npx --yes guildctl@0.4.0 config apply \
  ./guildcontrol.json \
  ./guildcontrol.candidate.json \
...
```sh
npx --yes guildctl@0.4.0 activity \
  --config ./guildcontrol.json \
...
```sh
npx --yes guildctl@0.4.0 recipe list
npx --yes guildctl@0.4.0 recipe enable message-channel ./guildcontrol.json \
  --channel-id YOUR_MESSAGE_CHANNEL_ID
npx --yes guildctl@0.4.0 recipe enable direct-messenger ./guildcontrol.json \
  --user-id EXPECTED_RECIPIENT_USER_ID
npx --yes guildctl@0.4.0 recipe enable guild-starter ./guildcontrol.json \
  --guild-id YOUR_GUILD_ID
```
...
```sh
npx --yes guildctl@0.4.0 recipe plan guild-starter ./guildcontrol.json \
  --guild-id YOUR_GUILD_ID
npx --yes guildctl@0.4.0 recipe apply guild-starter ./guildcontrol.json \
  --guild-id YOUR_GUILD_ID \
  --plan-digest SHA256_FROM_THE_PLAN \
