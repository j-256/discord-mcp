...
New feature policy follows the same document shape. For example, reviewed native bulk guild bans use `capabilities.bulkBanAudit`, `capabilities.bulkBans`, `scopes.bulkBanGuildIds`, `scopes.protectedUserIds`, and the `bulk-bans` toolset. Reviewed non-exact guild pruning separately uses `capabilities.guildPruneAudit`, `capabilities.guildPrunes`, `scopes.guildPruneGuildIds`, the optional `scopes.guildPruneIncludeRoleIds`, `limits.guildPruneMaxMembers`, and the `guild-prunes` toolset. No equivalent environment-policy interface, legacy alias, fallback parser, or migration layer exists.
...
Keep first setup read-only, then inspect and plan an additive recipe when a write workflow is needed. `guild-builder` adds model-neutral authoring, two-pass capture, and caller-retained blueprints for exact guild structure, profile, settings, Welcome Screen, onboarding, staged AutoMod policy, and static publications without storing the objective or blueprint. When served, it uses a nonprivileged `GUILDS`-only layout-evidence connection for settings and onboarding while leaving the configured event feed unchanged. `incident-response` adds privacy-minimized incident audit and reviewed time-bounded lockdown changes with no Gateway or privileged intent. `channel-publisher` adds bounded text and static Components V2 publication and reports the required Message Content intent. [`direct-messenger`](docs/reference.md#exact-one-to-one-private-message-lifecycle) adds exact-user reads plus reviewed text and static Components V2 delivery, same-format edits, and deletion. Optional private-file delivery requires a separate capability and owned attachment root; the recipe does not grant it. Private messaging adds no guild permission, privileged intent, Gateway feed, group DM, recipient discovery, remote-file fetch, interactive callback, or immediate write path.
...
| --- | --- |
| Discovery and reads | Local exact-ID conversion from canonical jump links and typed mentions, guilds, channels, exact allowlisted one-to-one private messages, current-application command, linked-role schema, SKU catalog, and guild-wide credential-redacted webhook exposure audits, global and guild voice regions, exact transient voice-channel status, roles, effective permissions, member-safe audits, message history, indexed search, threads, forums, polls, reactions, audit history, integrations, invites, templates, guild and application-owned emojis, stickers, soundboard, events, Stage instances, onboarding, Welcome Screens, profiles, settings, incident actions, and channel webhooks |
