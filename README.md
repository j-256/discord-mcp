...
Treat attachment messages, forum posts, message-pin changes, channel permission-overwrite changes, guild scaffolds, channel creation, role creation, message deletion, and member moderation as consequential even though the connector records only bounded identifiers and outcomes.
...
| `DISCORD_MCP_AUDIT_FILE` | No | Activity JSONL path; attachment, channel, forum-post, guild-scaffold, message-pin, permission-overwrite, and role operation receipts use an adjacent private directory; defaults under the user's local state directory |
...
| `list_activity` | Local read | Read content-free attachment, channel-creation, forum-post, scaffold-step, role-creation, message-pin, permission-overwrite, deletion, interaction, and member-moderation activity |
