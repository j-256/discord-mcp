...
Keep first setup read-only, then plan one additive recipe. `guild-starter` adds reviewed public layouts with nonprivileged `GUILDS` evidence; `guild-builder` adds the broader blueprint lifecycle. `incident-response` adds reviewed time-bounded lockdown. `channel-publisher` adds text, Components V2, and remote-free embeds; authenticated request Buttons additionally require the exact native Interaction policy and managed command. [`direct-messenger`](docs/reference.md#exact-one-to-one-private-message-lifecycle) adds exact-user reads and reviewed delivery, editing, and deletion. Each recipe reports permissions, intents, scopes, risks, and exclusions before changing policy.
...
| --- | --- |
| Discovery and reads | Complete ID-only bot-installation drift, exact-ID jump-link and mention parsing, guilds, channels, allowlisted private messages, Activity-instance verification, voice regions and transient channel status, roles, effective permissions, member-safe audits, message history, bounded exact-message reply collection, native filtered search, privacy-safe vague-memory conversation recall with fresh context, native attachment reads, aggregate activity, threads, forums, polls, reactions, audit history, integrations, invites, templates, guild/application emojis, stickers, soundboard, events, Stage instances, onboarding, Welcome Screens, profiles, settings, Community routing, incident actions, and channel webhooks |
| Messages and communities | Idempotent guild and webhook delivery; exact-message Discord-native task coordination with a static playbook and bounded inspection prompt; reviewed exact-recipient private messages; restart-verifiable Components V2 with callback-free links or authenticated private-request Buttons; remote-free embeds; attachments and transient reads; pins, bounded additive reaction sets, crossposts, subscriptions, forwarding, forums, threads, guided create-inspect-end native polls, and exact deletion |
...
| Operations | Progressive discovery, resources, prompts, exact-ID completion, display-only plan review, a model-neutral Discord coordination playbook, strict non-secret policy and profiles, generated host guides, presets and recipes, privacy-safe audits, content-free activity, durable write coordination, bounded Gateway events, token-private slash-command and request-button continuations, and local or W3C-parented OpenTelemetry diagnostics |
