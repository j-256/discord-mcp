...
| --- | --- |
| Discovery and reads | Local exact-ID conversion from canonical jump links and typed mentions, guilds, channels, exact allowlisted one-to-one private messages, current-application command exposure, global and guild voice regions, exact transient voice-channel status, roles, effective permissions, member-safe audits, message history, indexed search, threads, forums, polls, reactions, audit history, integrations, invites, templates, guild and application-owned emojis, stickers, soundboard, events, Stage instances, onboarding, Welcome Screens, profiles, settings, incident actions, and webhooks |
