...
Keep first setup read-only, then plan one additive recipe. `guild-starter` adds reviewed public layouts with nonprivileged `GUILDS` evidence; `guild-builder` adds the broader blueprint lifecycle. `incident-response` adds reviewed time-bounded lockdown. `channel-publisher` adds text, static Components V2, and remote-free embeds. [`direct-messenger`](docs/reference.md#exact-one-to-one-private-message-lifecycle) adds exact-user reads and reviewed delivery, editing, and deletion. Each recipe reports permissions, intents, scopes, risks, and exclusions before changing policy.
...
| --- | --- |
| Discovery and reads | Complete ID-only bot-installation drift, exact-ID jump-link and mention parsing, guilds, channels, allowlisted private messages, Activity-instance verification, voice regions and transient channel status, roles, effective permissions, member-safe audits, message history, native filtered search, privacy-safe vague-memory conversation recall with fresh context, native attachment reads, aggregate activity, threads, forums, polls, reactions, audit history, integrations, invites, templates, guild/application emojis, stickers, soundboard, events, Stage instances, onboarding, Welcome Screens, profiles, settings, Community routing, incident actions, and channel webhooks |
| Messages and communities | Command-bound processing signals, idempotent guild bot sends and edits, reviewed exact-recipient private-message text, file, and static Components V2 lifecycle, credential-private webhook reads and reviewed delivery lifecycle, exact pins, reactions, announcement crossposts and subscriptions, immutable forwarding, reviewed local-file attachment sends, transient exact attachment reads, typed announcement, incident, poll-result, release-note, and welcome-card compilation, reviewed restart-verifiable Components V2 and remote-free embeds, forum posts, thread creation, native polls, and exact deletion |
| Guild structure | Four deterministic public starters, custom authoring, and two-pass capture into caller-retained blueprints covering additive structure, exact role and channel configuration, bottom-up exact or receipt-bound role and channel ordering with explicit reparenting acknowledgement, exact-channel one-target permission overwrites, profile, settings, monotonic Community, complete Welcome Screens and onboarding, staged AutoMod, and restart-verifiable Components V2 publications; additive channels and roles; reviewed channel and standard-role retirement; resumable scaffolds; atomic channel cloning; relative channel placement within or across categories; boost-aware voice and Stage channel metadata; connection-sensitive voice-channel status changes; reviewed parent-category permission synchronization; forum tags; and reviewed local or Unicode role icons |
| Members and moderation | Privacy-minimized member and ban reads; exact nickname, named Membership Screening bypass, role, voice, connector and exact-member thread-membership, kick, ban, unban, and timeout workflows; reviewed exact-role batches with per-target evidence, checkpoints, and approved resumption; reviewed native bulk bans with complete target-set evidence and per-target readback; and bounded non-exact guild pruning with protected-identity role shields and count-only outcomes |
| Community configuration | Application command, linked-role, SKU, beneficiary entitlement, and subscription audits; test grants and consumable consumption; reviewed guild and global command lifecycles; native command management; reviewed application schema, intents, bot profile, and exact-guild connector departure; reviewed Community enablement and routing; Guild Templates; integrations; finite role-aware private-file invites; capability-safe invite audit and revocation; privacy-bounded vanity URL audit; credential-safe webhooks and private messages; onboarding; Welcome Screens; authenticated widget settings; time-bounded incident actions; application-owned emojis; guild expressions; soundboard; AutoMod; scheduled events; and Stage lifecycle |
| Operations | Full or progressive tool discovery, resources, prompts, policy-aware exact-ID completion, optional display-only MCP App plan review, strict non-secret policy files and managed profiles, private policy and host-activation guides, deterministic read-only presets, review-first additive policy recipes, privacy-safe application posture, command, linked-role schema, SKU catalog, and guild webhook exposure audits, digest-bound content-free activity review, durable cross-process write coordination, optional privacy-safe Gateway events with authenticated exact-route sparse sharding, shared session-start preflight, and per-shard outbound budgeting, token-private Interaction continuations, and local or W3C-parented OpenTelemetry diagnostics |
