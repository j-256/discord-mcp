...
| Guild structure | Four deterministic public starter layouts, custom authoring, and two-pass capture into caller-retained guild blueprints across additive structure, exact role configuration, bottom-up exact or receipt-bound role-order chains, exact channel metadata, exact-channel one-target permission overwrites, profile, settings, monotonic Community, complete Welcome Screens and onboarding, staged AutoMod, and ordered restart-verifiable static Components V2 publications; additive channels and roles; reviewed exact channel and standard-role retirement; resumable scaffolds; atomic channel cloning; relative channel placement within or across categories; boost-aware voice and Stage channel metadata; connection-sensitive voice-channel status changes; reviewed parent-category permission synchronization; forum tags; and reviewed local or Unicode role icons |
