...
const CONFIG_RECIPE_SOURCES = Object.freeze([
  {
    ...
    description: "Add exact-guild caller-retained blueprints for additive structure, guild profile, named settings, complete Welcome Screen, and complete onboarding through one reviewed frontier per call.",
    gateway: {
    ...
    privilegedIntents: [],
    risks: [
      "Blueprint execution can create roles and channels or replace guild profile, settings, Welcome Screen, and onboarding state",
    ...
    warnings: [
      ...
      "Static Components V2 publications remain unavailable until exact channels are added through the channel-publisher recipe",
    ],
  },
  {
    botPermissions: [
    ...
    description: "Add bounded message reads plus safe plain-text and reviewed static Components V2 publication for exact channels or their documented active-thread scope.",
    gateway: {
    ...
    risks: [
      "Message sends, reactions, and component creates make visible Discord changes",
      "Message and component edits replace exact bot-authored state and can remove omitted content",
    ],
    scope: {
      kind: "channel",
