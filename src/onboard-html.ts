...
function copyButton(id: string, label: string): string {
  return `<button type="button" data-copy="${escapeHtml(id)}" data-label="${escapeHtml(label)}">Copy</button>`
}

function codeBlock(id: string, value: string, label: string): string {
  return `<div class="copy-row"><pre id="${escapeHtml(id)}"><code>${escapeHtml(value)}</code></pre>${copyButton(id, label)}</div>`
}
...
export function renderOnboardHtml(value: OnboardReport): string {
  ...
  const recoveryCommands = [
    `npx --yes guildctl@${CONNECTOR_VERSION} doctor --config ${JSON.stringify(report.configFile)} --online`,
    `npx --yes guildctl@${CONNECTOR_VERSION} smoke --config ${JSON.stringify(report.configFile)}`,
    `npx --yes guildctl@${CONNECTOR_VERSION} host --npx --config ${JSON.stringify(report.configFile)} --adapter ${report.host.id === "claude-desktop" ? "mcp-json" : report.host.id}`,
  ].join("\n")
  return `<!doctype html>
<html lang="en">
<head>
...
<body>
  ...
  <main class="shell" id="main">
    ...
    <details class="evidence"><summary>Inspect complete credential-free evidence</summary><pre><code>${escapeHtml(evidence)}</code></pre></details>
  </main>
