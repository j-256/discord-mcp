...
function toolMarkup(
  ...
  <details id="tool-${escapeHtml(tool.name)}">
    <summary>
      ...
      <span class="badges"><span class="badge risk-${escapeHtml(metadata.risk)}">${escapeHtml(displayName(metadata.risk))}</span><span class="badge">${escapeHtml(metadata.toolset)}</span></span>
    </summary>
    <div class="tool-body">
...
<body>
  ...
  <header class="shell hero">
    ...
    <p class="lede">A self-contained view of the exact MCP contract negotiated from version ${escapeHtml(report.serverVersion)}. This artifact cannot execute a tool, contact Discord, read a credential, open the Gateway, export telemetry, or create an activity record.</p>
    ...
    <div class="guarantees">
      <section class="panel"><h2>What this proves</h2><ul class="checks"><li>Production registrations negotiated</li><li>Complete tool annotations checked</li><li>Known and unknown calls blocked identically</li><li>Static safety guidance fingerprinted</li><li>No credential required</li><li>No Discord execution available</li></ul></section>
      <section class="panel"><h2>Risk distribution</h2>${riskBars(snapshot)}<p class="digest"><b>Contract</b><br>${escapeHtml(report.contractDigest)}<br><br><b>Safety</b><br>${escapeHtml(report.safetyResourceDigest)}</p></section>
    </div>
  </header>
  <nav class="sticky-nav" aria-label="Contract sections"><div class="shell nav-inner"><a href="#tools">Tools</a><a href="#prompts">Prompts</a><a href="#resources">Resources</a><a href="#instructions">Instructions</a><a href="#safety">Safety</a></div></nav>
