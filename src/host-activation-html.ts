...
<body>
  ...
  <main id="main" class="shell" tabindex="-1">
    ...
    <section class="step" id="adapters">
      ...
      <div class="step-body"><div class="adapter-grid" role="list" aria-label="Verified host adapters">${adapterCatalog.adapters.map(renderHostAdapter).join("")}</div><p class="callout"><strong>Merge, do not overwrite.</strong> Review the selected JSON, merge only its server and input records into the documented destination, then preserve every unrelated host entry. These projections do not inspect the destination or resolve conflicts for you.</p></div>
    </section>
