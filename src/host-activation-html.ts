...
export const HOST_ACTIVATION_HTML_FORMAT = "discord-mcp.host-activation-html.v1"
export const HOST_ACTIVATION_HTML_SCHEMA_VERSION = 1
...
<body>
  ...
  <header class="shell hero">
    ...
    <p class="lede">This credential-free artifact turns one verified Discord MCP policy into an exact local stdio handoff. It gives any compatible host the same fields without guessing a client schema, writing host configuration, or putting a bot token in this page.</p>
    ...
    <div class="boundary">
      ...
      <section class="panel" aria-labelledby="limits-title"><h2 id="limits-title">What this artifact cannot do</h2><ul class="checks"><li>Cannot discover or edit an MCP host</li><li>Cannot read a credential value</li><li>Cannot start a local process</li><li>Cannot contact Discord or another network</li><li>Cannot validate a host-specific translation</li><li>Cannot prove a host approval interface</li></ul></section>
    </div>
  </header>
  <div class="sticky"><section class="shell progress-row" aria-label="Activation progress"><strong>Local activation checklist</strong><progress id="activation-progress" value="0" max="5" aria-label="Host activation progress"></progress><span id="progress-text" aria-live="polite">0 checks completed</span></section></div>
  <main id="main" class="shell" tabindex="-1">
    ...
    <section class="step" id="verify">
      <div class="step-head"><div><h2>4. Restart and verify without writing</h2><p>Reload the host, inspect its negotiated server, then use the exact read-only request below.</p></div><label class="done"><input type="checkbox" data-step autocomplete="off" aria-label="Verify activated host">Checked</label></div>
    ...
    <section class="step" id="recover">
      <div class="step-head"><div><h2>5. Isolate translation failures</h2><p>Use the same structured process outside the host before changing policy or Discord permissions.</p></div><label class="done"><input type="checkbox" data-step autocomplete="off" aria-label="Review activation recovery">Checked</label></div>
