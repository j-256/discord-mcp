    ...
    async (uri) => ({
      contents: [{
        ...
        text: [
          ...
          "",
          "Guild invite audit requires separate audit and exact-guild scope, verified connector identity, a complete bounded guild role inventory, a visibility-bounded channel snapshot, and complete MANAGE_GUILD evidence. Raw invite codes and URLs are bearer capabilities, so the connector replaces them with process-keyed opaque references before building any MCP result. Authenticated cursors bind a local page to the complete fresh projected invite inventory and fail when it changes. Reviewed creation is separately gated by exact channel and private-root allowlists, finite age and use limits, explicit bearer acknowledgement, complete direct-channel overwrite evidence, and exact VIEW_CHANNEL plus CREATE_INSTANT_INVITE authority without requiring MANAGE_GUILD. It fresh-checks an absent canonical direct-child output target, obtains signed approval, durably excludes the channel and guild invite collection, exclusively reserves a process-owned 0600 file, performs one non-retried unique-invite POST, strictly validates the response, writes and syncs the capability only to that file, and verifies exact identity through an unauthenticated lookup. The code and URL never enter MCP results, errors, diagnostics, observability, receipts, or activity records; ambiguous mutation, file, or verification boundaries remain quarantined without retry or automatic compensation. Reviewed revocation requires an additional toggle, fresh keyed plan, signed approval, one-shot reservation, pending content-free activity, one non-retried secret-route DELETE, returned-identity validation, and full-inventory absence readback. Codes, URLs, profiles, role names, audit reasons, raw keys, and raw payloads are never persisted or emitted through diagnostics.",
          "",
