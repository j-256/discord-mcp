    ...
    async (uri) => boundedResource({
      contents: [{
        ...
        text: [
          ...
          "",
          "One-to-one private messages are an explicit exact-user exception outside guild read scope. Every read requires one configured ordinary recipient plus caller-known channel and message identities, re-verifies pinned connector identity and the exact two participants, returns only transient content with bounded counts, and omits profiles, avatars, attachment URLs, raw payloads, discovery, group DMs, persistence, and DM Gateway events. Send planning reads only the exact user and never opens a DM channel. Send, reply, plain-text connector-message edit, and irreversible deletion use action-specific gates and acknowledgements, a transient content-bound keyed plan, signed approval, forced empty mentions, fixed anti-spam limits, durable exact user, channel, and message coordination, a request-bound schema-v2 content-free receipt, pending activity before contact, immutable channel and dispatch checkpoints, no automatic mutation retry or rollback, and exact state or absence readback. Ambiguous outcomes retain their claims and require verify_direct_message_change with the exact caller-retained request.",
          "",
