    ...
    async (uri) => boundedResource({
      contents: [{
        ...
        text: [
          ...
          "",
          "Exact-reference parsing accepts one complete canonical Discord channel or message jump link or official typed mention and returns only typed IDs, fixed syntax traits, privacy guarantees, and bounded local read-policy eligibility. It never scans prose, resolves a name, contacts Discord, persists or echoes the input, returns embedded command or emoji names, accepts invite, webhook, OAuth, CDN, attachment, or media links, verifies Discord access, or grants downstream authority. Every later tool still enforces its own exact scope and safety gates.",
          "",
          ...
          "",
          "One-to-one private messages are an explicit exact-user exception outside guild read scope. Every read requires one configured ordinary recipient plus caller-known channel and message identities, re-verifies pinned connector identity and the exact two participants, returns only transient plain text, normalized static Components V2, or bounded URL-free single-attachment metadata, and omits generated component IDs, profiles, avatars, attachment URLs, raw payloads, discovery, group DMs, persistence, and DM Gateway events. Send planning reads only the exact user and one explicitly requested owned local file when present, and never opens a DM channel. Private-file send and reply require an independent capability plus an owned attachment root and accept no URL, base64, multiple-file, edit, or download path. Send, reply, same-format connector-message edit, and irreversible supported-message deletion use action-specific gates and acknowledgements, a transient complete-body- and byte-bound keyed plan, signed approval, forced empty mentions, fixed anti-spam limits, durable exact user, channel, and message coordination, a request-bound schema-v2 content-free receipt that retains only format and nullable attachment size, pending activity before contact, immutable channel and dispatch checkpoints, no automatic mutation retry or rollback, and exact presentation, body, URL-free attachment metadata, or absence readback. Ambiguous outcomes retain their claims and require verify_direct_message_change with the exact caller-retained request without reopening or downloading the attachment.",
          "",
          ...
          "",
          "Deletion, member moderation, bulk guild bans, and guild pruning are review-first workflows. Planning is read-only. Execution remains a separate destructive tool and requires every configured policy, freshness, signed-state, approval, confirmation, and audit gate.",
        ].join("\n"),
        uri: uri.href,
      }],
    }),
