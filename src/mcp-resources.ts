    ...
    async (uri) => boundedResource({
      contents: [{
        ...
        text: [
          ...
          "",
          "One-to-one private messages are an explicit exact-user exception outside guild read scope. Every read requires one configured ordinary recipient plus caller-known channel and message identities, re-verifies pinned connector identity and the exact two participants, returns only transient plain text or normalized static Components V2 with deterministic previews and bounded counts, and omits generated component IDs, profiles, avatars, attachment URLs, raw payloads, discovery, group DMs, persistence, and DM Gateway events. Send planning reads only the exact user and never opens a DM channel. Plain-text or static Components V2 send, reply, same-format connector-message edit, and irreversible supported-message deletion use action-specific gates and acknowledgements, a transient complete-body-bound keyed plan, signed approval, forced empty mentions, fixed anti-spam limits, durable exact user, channel, and message coordination, a request-bound schema-v2 content-free receipt that retains only the format class, pending activity before contact, immutable channel and dispatch checkpoints, no automatic mutation retry or rollback, and exact presentation, body, or absence readback. Ambiguous outcomes retain their claims and require verify_direct_message_change with the exact caller-retained request.",
          "",
