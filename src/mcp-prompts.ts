    ...
    ({ objective }) => userPrompt(
      promptText(
        ...
        [
          ...
          "5. Never invent or fuzzy-resolve an exact Discord identifier, identity, permission, scope, live-state fact, audit reason, acknowledgement, confirmation, request state, or plan digest. Copy a caller-supplied exact ID or canonical Discord reference only into a matching validated argument; use parse_discord_reference when the revealed route requires local reference parsing. Treat names as display evidence only. Ask for every missing required authority or intent value instead of filling it in. A one-shot operation or idempotency key is bookkeeping rather than authority: when the exact revealed planner contract requires one and the caller omitted it, generate a fresh opaque unique key locally, retain it unchanged for the plan and any later separately approved execution, never reuse it after reservation, and do not ask the caller to invent or confirm it.",
    ...
    (input) => userPrompt(
      promptText(
        ...
        [
          ...
          "5. Stay within additive bounded scaffold structure, optional top-to-bottom roleOrder and channelOrders containing only explicitly requested new scaffold references, sparse guild profile and named settings, explicitly acknowledged monotonic Community enablement and exact routing, complete Welcome Screen and onboarding replacement, staged AutoMod, and ordered static Components V2 publications. Omit exact existing-role configuration, channel metadata, and channel permission overwrites because this offline prompt cannot verify their target IDs; the caller may add those phases through separately reviewed JSON after obtaining authoritative IDs and configuring every standalone scope. Include roleOrder only when the objective states the exact desired role chain. Include each channelOrders chain only when the objective states the exact desired category or same-parent child chain; never set acknowledgeReparenting in this offline workflow. Preserve every semantic order. Include a Community phase only when every routing target is an exact verified ID or an explicitly requested new text-channel key, and never infer Community state from the objective. Never add ADMINISTRATOR, adopt a managed role, delete resources, assign roles, add callback-bearing components, attachments, or replies, import a remote or model-invented canned template, use fuzzy matching or managed markers, or pad the manifest with unwanted resources merely to satisfy its schema.",
          ...
          "7. Stop after drafting. Do not call preview_guild_blueprint, capture_guild_blueprint, plan_guild_blueprint, execute_guild_blueprint, or verify_guild_blueprint. Tell the user to inspect the candidate, pass it separately through preview_guild_blueprint for complete local intent review, and invoke the separate review_guild_blueprint prompt only after every field, dependency, omission, and exact reference is acceptable.",
        ],
      ),
    ...
    (input) => userPrompt(
      promptText(
        ...
        [
          ...
          "4. Present the complete caller-retained blueprint, capture digest and window, coverage, omissions, privacy projection, limitations, and recovery-binding count. State that capture does not infer existing-role configuration, role-order, channel-metadata, channel-order, or permission-overwrite convergence phases. If channelId or roleId was supplied, isolate only the binding with the exact matching resource type and ID; if no exact match exists, report `Target recovery binding unavailable` and stop. If no target was supplied, present every returned binding without inventing a preferred target.",
  ...
  if (toolsets.has("guild-blueprints")) server.registerPrompt(
    ...
    ({ requestJson }) => userPrompt(
      promptText(
        ...
        [
          "1. Call only plan_guild_blueprint with the exact caller-retained input object. Do not invoke preview_guild_blueprint inside this prompt; recommend that separate local preview before this workflow when its complete intent sequence has not already been inspected.",
          ...
          "3. Present the verified application, bot, guild, complete manifest preview overlay with freshly assessed and deferred entries, planner-discovered prerequisites, fixed additive structure, exact-ID role configuration, bottom-up role-order adjacency, exact-ID channel metadata, bottom-up channel-order adjacency, exact-channel permission-overwrite convergence, profile, settings, Community, Welcome Screen, onboarding, staged AutoMod, and ordered static publication phase sequence, exact single executable frontier or content-free channel-ordering, Community, AutoMod, or publication blocker, symbolic-to-exact resource bindings, nested domain plan, phase-, exact-target-, role-adjacency-, channel-adjacency-, AutoMod-stage-, or publication-key-separated operation hashes, caller-retained request digest, privacy boundary, warnings, creation time, and aggregate keyed plan digest for review. Never describe a deferred entry or possible local-preview stage as assessed live. For role configuration, review the requested known permission set or deltas, unknown-bit preservation, hierarchy, holder impact, and Administrator prohibition. For role ordering, review the top-to-bottom chain, exact resolved target and anchor, affected hierarchy segment, aggregate holder impact, and hierarchy-sensitive permissions. For channel metadata, review every requested sparse field and type-applicable permission. For channel ordering, review the top-to-bottom chain, exact resolved target and anchor, same- or cross-parent mode, complete affected groups and capacity, movement authority, overwrite preservation, and explicit reparenting acknowledgement when required. For a permission overwrite, review the exact separately allowlisted channel, exact member or exact or receipt-bound role target, named delta or deletion, before and after access, connector continuity, and complete permission evidence. For Community, review the explicit acknowledgement, exact routing IDs, preserved feature digest, and whether temporary guild ownership or complete Administrator authority is needed for enablement instead of Manage Guild for routing only.",
          "4. Treat any domain scope, Message Content intent, identity, permission, hierarchy, capacity, receipt conflict, uncertainty, drift, spent operation binding, unresolved scaffold channel or role reference, unacknowledged live channel reparenting, Community dependency, AutoMod or publication blocker, or changed request digest as a blocker. A completed nested role, channel, ordering, profile, settings, Community, Welcome Screen, or onboarding receipt may satisfy only fresh exact matching state; later drift must remain a spent-key conflict. AutoMod create recovery must use only a matching request-bound receipt and exact rule ID; publication recovery must use only the exact receipt-bound message. Never use fuzzy names, inventory positions, or message history for recovery.",
          ...
          "7. When authoring a separate manifest from live guild state, capture_guild_blueprint may provide a two-pass caller-retained draft. Capture does not add existing-role configuration, role-order, channel-metadata, channel-order, or permission-overwrite convergence phases. Never pass a review-required capture to planning until every omission and exact-bound reference has been explicitly reviewed and the partial desired state has been accepted or edited.",
        ],
      ),
