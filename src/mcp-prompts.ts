    ...
    (input) => userPrompt(
      promptText(
        ...
        [
          ...
          "4. Present the complete caller-retained blueprint, capture digest and window, coverage, omissions, privacy projection, limitations, and recovery-binding count. State that capture does not infer existing-role configuration, role-order, channel-metadata, or permission-overwrite convergence phases. If channelId or roleId was supplied, isolate only the binding with the exact matching resource type and ID; if no exact match exists, report `Target recovery binding unavailable` and stop. If no target was supplied, present every returned binding without inventing a preferred target.",
    ...
    ({ requestJson }) => userPrompt(
      promptText(
        ...
        [
          ...
          "3. Present the verified application, bot, guild, complete manifest preview overlay with freshly assessed and deferred entries, planner-discovered prerequisites, fixed additive structure, exact-ID role configuration, bottom-up role-order adjacency, exact-ID channel metadata, exact-channel permission-overwrite convergence, profile, settings, Community, Welcome Screen, onboarding, staged AutoMod, and ordered static publication phase sequence, exact single executable frontier or content-free Community, AutoMod, or publication blocker, symbolic-to-exact resource bindings, nested domain plan, phase-, exact-target-, role-adjacency-, AutoMod-stage-, or publication-key-separated operation hashes, caller-retained request digest, privacy boundary, warnings, creation time, and aggregate keyed plan digest for review. Never describe a deferred entry or possible local-preview stage as assessed live. For role configuration, review the requested known permission set or deltas, unknown-bit preservation, hierarchy, holder impact, and Administrator prohibition. For role ordering, review the top-to-bottom chain, exact resolved target and anchor, affected hierarchy segment, aggregate holder impact, and hierarchy-sensitive permissions. For channel metadata, review every requested sparse field and type-applicable permission. For a permission overwrite, review the exact separately allowlisted channel, exact member or exact or receipt-bound role target, named delta or deletion, before and after access, connector continuity, and complete permission evidence. For Community, review the explicit acknowledgement, exact routing IDs, preserved feature digest, and whether temporary guild ownership or complete Administrator authority is needed for enablement instead of Manage Guild for routing only.",
          ...
          "7. When authoring a separate manifest from live guild state, capture_guild_blueprint may provide a two-pass caller-retained draft. Capture does not add existing-role configuration, role-order, channel-metadata, or permission-overwrite convergence phases. Never pass a review-required capture to planning until every omission and exact-bound reference has been explicitly reviewed and the partial desired state has been accepted or edited.",
        ],
      ),
