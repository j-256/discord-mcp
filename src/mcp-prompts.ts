...
export function registerDiscordPrompts(
    ...
    (input) => userPrompt(
      promptText(
        ...
        [
          ...
          "5. Stay within additive bounded scaffold structure, sparse guild profile and named settings, explicitly acknowledged monotonic Community enablement and exact routing, complete Welcome Screen and onboarding replacement, staged AutoMod, and ordered static Components V2 publications. Include a Community phase only when every routing target is an exact verified ID or an explicitly requested new text-channel key, and never infer Community state from the objective. Never add ADMINISTRATOR, adopt a managed role, create permission overwrites, reorder or delete resources, assign roles, add callback-bearing components, attachments, or replies, import a canned or remote template, use fuzzy matching or managed markers, or pad the manifest with unwanted resources merely to satisfy its schema.",
  ...
  if (toolsets.has("guild-blueprints")) server.registerPrompt(
    MCP_PROMPT_NAMES.reviewGuildBlueprint,
    {
      ...
      description: "Create and review the next fixed-order frontier of one caller-retained Discord guild blueprint, including monotonic Community state and receipt-bound staged AutoMod policy, without executing it.",
    ...
    ({ requestJson }) => userPrompt(
      promptText(
        ...
        [
          ...
          "3. Present the verified application, bot, guild, fixed structure, profile, settings, Community, Welcome Screen, onboarding, staged AutoMod, and ordered static publication phase sequence, exact current frontier or content-free Community, AutoMod, or publication blocker, symbolic-to-exact resource bindings, nested domain plan, phase-, AutoMod-stage-, or publication-key-separated operation hashes, caller-retained request digest, privacy boundary, warnings, creation time, and aggregate keyed plan digest for review. For Community, review the explicit acknowledgement, exact routing IDs, preserved feature digest, and whether temporary guild ownership or complete Administrator authority is needed for enablement instead of Manage Guild for routing only.",
          "4. Treat any domain scope, Message Content intent, identity, permission, hierarchy, capacity, receipt conflict, uncertainty, drift, spent operation binding, unresolved scaffold channel or role reference, Community dependency, AutoMod or publication blocker, or changed request digest as a blocker. AutoMod create recovery must use only a matching request-bound receipt and exact rule ID; publication recovery must use only the exact receipt-bound message. Never use fuzzy names, inventory positions, or message history for recovery.",
